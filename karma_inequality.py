...
def get_fraction_percentile(karma):
    for x in np.arange(0,1,0.1):  
        a = proportion_of_karma(karma,x,strict=True)
        b = proportion_of_karma(karma,x+0.1,strict=True)
        x_fraction = a - b
        yield x_fraction
...
def get_karma_bar(karma):
    percentiles = list(get_fraction_percentile(karma))

    plt.figure()
    bar_width = 0.5
    per_ids   = np.array((range(len(percentiles))))
    plt.bar(per_ids, percentiles, bar_width)
    plt.title("Сколько кармы приходится на i-ый перцентиль пользователей?", fontsize=27)
    plt.tick_params(axis='both', which='major', labelsize=20)
